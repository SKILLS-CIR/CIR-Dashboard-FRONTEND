...
import { Plus, Pencil, Trash2, ChevronLeft, ChevronRight, Search, FolderOpen, ChevronDown, User, Calendar as CalendarIcon } from "lucide-react"
...
export default function ManagerAssignmentsPage() {
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            {/* Manage Responsibility Section */}
            <Card>
                <CardHeader>
                    <CardTitle>Manage Responsibility Timelines</CardTitle>
                    <CardDescription>
                        Update start and end dates for existing responsibilities
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="space-y-2">
                        {responsibilities.length === 0 ? (
                            <p className="text-muted-foreground text-center py-8">
                                No responsibilities available to manage.
                            </p>
                        ) : (
                                ...
                                {responsibilities.map((resp) => (
                                    ...
                                    >
                                        <div className="flex-1">
                                            <h4 className="font-medium">{resp.title}</h4>
                                            <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                                                <span>Cycle: {resp.cycle}</span>
                                                {resp.startDate && (
                                                    <span className="flex items-center gap-1">
                                                        <CalendarIcon className="h-3 w-3" />
                                                        Start: {new Date(resp.startDate).toLocaleDateString()}
                                                    </span>
                                                )}
                                                {resp.endDate && (
                                                    <span className="flex items-center gap-1">
                                                        <CalendarIcon className="h-3 w-3" />
                                                        End: {new Date(resp.endDate).toLocaleDateString()}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                        {/* TODO: Re-enable when ManageResponsibilityDialog component is created
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => openManageResponsibilityDialog(resp)}
                                        >
